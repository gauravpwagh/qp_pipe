...
| `image_regions` | the regions the crop image was cut from: `{regions: [{page, box: [x0,y0,x1,y1]}, ...], marks?: [...]}` in source-page pixel coordinates, in stacking order - one region per column/page the question spans. Set by the pipeline, rewritten by the Review UI's "Edit image" tool (so it opens on the automatic selection), which can add regions on other pages when a question continues overleaf; a job edited before pages could differ stores the older `{page, boxes}` shape, still read. `marks` (see "Formulas, chemistry and diagrams" below) each carry the `page` they were drawn on |
